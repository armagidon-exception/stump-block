...
class LoopStateHandler(StateHandler):
    def _handle(self, context: TraverseContext):
        ...
        if (
            current_state.type == State.PRE_LOOP
            or current_state.type == State.POST_LOOP
            or current_state.type == State.PARAMETER_LOOP
        ):
            print(context.current_node.type, context.enter, context.current_node.text)
